...
export function pickFoodHaptic(kind, { blastCount = 1 } = {}) {
  const h = CONFIG.homeFoods?.haptics ?? {};
  if (kind === 'tapBlast') {
    if (blastCount >= 4) return h.tapBlastBig ?? [34, 18, 38, 22, 26, 14];
    if (blastCount >= 2) return h.tapBlastChain ?? [22, 12, 28, 16, 20];
    return h.tapBlast ?? [16, 10, 26, 14, 18];
  }
  if (kind === 'slice') {
    const pool = [h.slice, h.sliceAlt1, h.sliceAlt2].filter((p) => p?.length);
    if (!pool.length) return [12, 22, 14];
    return pool[Math.floor(Math.random() * pool.length)];
  }
  if (kind === 'sliceBad') return h.sliceBad ?? [32, 20, 28, 14];
  if (kind === 'collectBad') return h.collectBad ?? [24, 14, 18];
  return h.collectGood ?? [10, 16, 8];
}
