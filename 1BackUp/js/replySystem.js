...
export function createReplySystem({ elements, getHighlightButton }) {
  ...
  function showActionPrompt(text, buttonKey = 'run') {
    ...
    if (actionPromptText) actionPromptText.textContent = text;
    actionPrompt?.removeAttribute('hidden');
    actionPrompt?.setAttribute('aria-hidden', 'false');
    actionPrompt?.classList.add('is-visible');
